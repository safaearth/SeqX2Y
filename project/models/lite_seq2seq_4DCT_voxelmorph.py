...
Author: Ouyang Hao
File: lite_seq2seq_4DCT_voxelmorph.py
-----
Comment:
...
class EncoderDecoderConvLSTM(nn.Module):
    ...
    def autoencoder(self, x, seq_len, rpm_x, rpm_y, future_step, h_t4, c_t4, h_t5, c_t5):
    # def autoencoder(self, x, seq_len, rpm_x, rpm_y, future_step, h_t4, c_t4):
        latent = []
        out = []
        # encoder
        e1 = []
        ...
        for t in range(future_step):
            ...
            # h_t4, c_t4 = self.ConvLSTM3d1(input_tensor = encoder_vector, # New add
            #                        cur_state = [h_t4,c_t4])

            h_t5, c_t5 = self.ConvLSTM3d2(input_tensor = encoder_vector, # c_t5 h_t5.shape=>[1,96,35,60,70]  input:(nf=96, in_chan=1, size1=70, size2=120, size3=140)
                                   cur_state = [h_t5,c_t5])
            ...
            encoder_vector = h_t5 # delete 1 convlstm open this
            latent += [h_t5]
    ...
    def forward(self, x, rpm_x, rpm_y, future_seq=0, hidden_state=None):
        ...
        outputs = self.autoencoder(x, seq_len, rpm_x, rpm_y, future_seq, h_t4, c_t4, h_t5, c_t5) # delete 1 convlstm open this
